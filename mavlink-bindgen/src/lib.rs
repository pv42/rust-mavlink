    ...
    match xml_definitions {
        XmlDefinitions::Files(files) => {
            ...
            for file in files {
                ...
                bindings.push(generate_single_file(file, destination_dir, format)?);
            }
        }
        XmlDefinitions::Directory(definitions_dir) => {
            ...
            for entry_maybe in read_dir(definitions_dir).map_err(|source| {
                ...
                bindings.push(generate_single_file(entry.path(), destination_dir, format)?);
            }
        }
    }
...
fn generate_single_file<P1: AsRef<Path>, P2: AsRef<Path>>(
    ...
    // codegen
    parser::generate(definitions_dir, &definition_filename, &mut outf, format)?;
